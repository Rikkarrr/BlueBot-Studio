# -*- coding: utf-8 -*-
# TinaBot — Tina's Mindrealm (fullscreen, robust confirm + spam)
#
# Flow (TL;DR):
...
# 9) POST_LEAVE_CHECK -> short window after leave: first try CONFIRM, else look for F_MINDREALM
# 10) Reset / Loop
#
...
TEMPLATES = {
    # World/Lobby (Tina's Mindrealm)
    "F_MINDREALM":        "assets/f_mindrealm.png",      # <- Tina's Mindrealm
    "BTN_TINA_HARD":      "assets/btn_tina_hard.png",    # <- Tina's Mindrealm
...
class Bot:
    def __init__(self):
        ...
        # one-time probe after 5s without F_MINDREALM in LEAVE_PARTY (Tina's Mindrealm)
    ...
    # --- One-time asset probe (debug, Tina's Mindrealm) ---
    def _probe_any_asset(self, sct) -> bool:
        # Order: scan quick/global items first
        probe_order = [
            "F_MINDREALM", "BTN_TINA_HARD", "BTN_MATCH",
    ...
    def loop(self):
        with mss.mss() as sct:
            ...
            while not self.exit_flag:
                ...
                # Active ONLY when we are NOT in LEAVE_PARTY and no dungeon HUD is visible.
                in_dungeon = self._scan_full(
                    sct, "HUD_DUNGEON", THRESH["HUD_DUNGEON"])
                if (self.state != "LEAVE_PARTY") and (not in_dungeon):
                    fg = self._scan_full(
                        sct, "F_MINDREALM", THRESH["F_MINDREALM"])
                    if fg and self.state != "WAIT_F":
                        print(
                            "[Bot] F-Guard: F_MINDREALM visible -> reset to WAIT_F")
                        self._full_reset()
                        time.sleep(0.15)
                        continue
                ...
                if self.state == "WAIT_F":
                    f = self._scan_full(sct, "F_MINDREALM",
                                        THRESH["F_MINDREALM"])
                    if f:
                        print("[Bot] F_MINDREALM prompt -> press F")
                        self._press_f()
                        time.sleep(1)
                ...
                # 2) Hard (one shot)
                    ...
                    if not self.hard_clicked:
                        h = self._scan_full(
                            sct, "BTN_TINA_HARD", THRESH["BTN_TINA_HARD"])
                        if h:
                            print("[Bot] HARD (Tina) -> click once")
                ...
                # 4) Captain dialog
                    ...
                    if (time.time() - self.captain_wait_start) >= CAPTAIN_DECLINE_GRACE:
                        print(
                            "[Bot] Captain prompt missing -> ESC, then I -> open party panel")
                        pyautogui.press('esc')
                        time.sleep(0.2)
                        pyautogui.press('i')
                ...
                # 5) Matching/Confirm loop
                    ...
                    if (not self.matching_seen) and ((now - self.match_check_start) > 5.0):
                        print(
                            "[Bot] No Matching/Confirm -> ESC, then I -> open party panel")
                        pyautogui.press('esc')
                        time.sleep(0.2)
                        pyautogui.press('i')
                        self.leave_enter_ts = time.time()
                ...
                # 9) Post-leave sanity
                    ...
                    if cfm:
                        print(
                            "[Bot] Post-Leave: CONFIRM found -> click & back to CONFIRM_MONITOR")
                        self._click_at(cfm.center)
                        time.sleep(0.6)
                        self.confirm_monitor_start = time.time()
                        self.last_confirm_seen = time.time()
                    ...
                    if f:
                        print(
                            "[Bot] Post-Leave: F_MINDREALM detected -> back to WAIT_F")
                        self._full_reset()
                        continue
                ...
                # 10) Leave-Party flow
                    ...
                    # 2) After 5s without leave icon: ESC -> check F_MINDREALM
                    if (now - self.leave_enter_ts) >= 5.0:
                        print(
                            "[Bot] Leave-Icon not found for 5s -> press ESC and check F_MINDREALM")
                        pyautogui.press('esc')
                        time.sleep(0.2)
                        ...
                        if f:
                            print(
                                "[Bot] F_MINDREALM detected after ESC -> reset to WAIT_F")
                            self._full_reset()
                            continue

                        # one-time asset probe after 5s without F (Tina's Mindrealm)
