# -*- coding: utf-8 -*-
# KanamiaBot — Kanamia's Mindrealm (fullscreen, robust confirm + spam)
#
# Flow (TL;DR):
...
# 9) POST_LEAVE_CHECK -> short window after leave: first try CONFIRM, else look for F_KANAMIA
# 10) Reset / Loop
#
...
import os  # env overrides from BlueBot GUI
import numpy as np
import cv2 as cv
import mss
import pyautogui
...
TEMPLATES = {
    # World/Lobby (Kanamia's Mindrealm)
    "F_KANAMIA":          "assets/f_kanamia.png",        # <- Kanamia's Mindrealm
    "BTN_KANAMIA_HARD":   "assets/btn_kanamia_hard.png",  # <- Kanamia's Mindrealm
...
class Vision:
    ...
    def find_best(self, frame_bgr, key, thr: float):
        ...
        if H < tmpl.shape[0] or W < tmpl.shape[1]:
            if DEBUG:
                print(
                    f"[Skip] {key} (Kanamia): {W}xH {W}x{H} < tmpl {tmpl.shape[1]}x{tmpl.shape[0]}")
            return None
        res = cv.matchTemplate(gray, tmpl, cv.TM_CCOEFF_NORMED)
        _, maxV, _, maxL = cv.minMaxLoc(res)
        if DEBUG:
            print(f"[Score] {key} (Kanamia): {maxV:.3f}")
        if maxV >= thr:
            x, y = maxL
...
class Bot:
    def __init__(self):
        ...
        # one-time probe after 5s without F_KANAMIA in LEAVE_PARTY (Kanamia's Mindrealm)
    ...
    def _maybe_arm_hud_timer(self, sct):
        if not self.hud_timer_armed:
            ...
            if hud:
                ...
                print(
                    "[Bot] HUD detected (Kanamia) -> arm 16min timer + press H once")
                self._press_h_once()
    ...
    # --- One-time asset probe (debug, Kanamia's Mindrealm) ---
    def _probe_any_asset(self, sct) -> bool:
        # Order: scan quick/global items first
        probe_order = [
            "F_KANAMIA", "BTN_KANAMIA_HARD", "BTN_MATCH",
        ...
        for k in probe_order:
            ...
            if m:
                print(
                    f"[Probe] Found asset (Kanamia): {k} (score={m.score:.3f})")
                return True
        print("[Probe] No asset detected (Kanamia).")
        return False
    ...
    def loop(self):
        with mss.mss() as sct:
            full = _monitor_bbox(sct, self.window_rect)
            print(
                f"[Bot] Started (Kanamia). State=WAIT_F | monitor={MONITOR_INDEX} box={full}")
            while not self.exit_flag:
                ...
                if (self.state != "LEAVE_PARTY") and (not in_dungeon):
                    fg = self._scan_full(sct, "F_KANAMIA", THRESH["F_KANAMIA"])
                    if fg and self.state != "WAIT_F":
                        print(
                            "[Bot] F-Guard: F_KANAMIA visible (Kanamia) -> reset to WAIT_F")
                        self._full_reset()
                        time.sleep(0.15)
                        continue
                ...
                if self.state == "WAIT_F":
                    f = self._scan_full(sct, "F_KANAMIA", THRESH["F_KANAMIA"])
                    if f:
                        print("[Bot] F_KANAMIA prompt (Kanamia) -> press F")
                        self._press_f()
                        time.sleep(1)
                ...
                # 2) Hard (one shot)
                    ...
                    if not self.hard_clicked:
                        h = self._scan_full(
                            sct, "BTN_KANAMIA_HARD", THRESH["BTN_KANAMIA_HARD"])
                        if h:
                            print("[Bot] HARD (Kanamia) -> click once")
                ...
                # 3) Match (one shot)
                    ...
                    if not self.match_clicked:
                        ...
                        if m:
                            print("[Bot] MATCH (Kanamia) -> click once")
                ...
                # 4) Captain dialog
                    ...
                    if d:
                        print("[Bot] Captain prompt (Kanamia): Decline -> click")
                    ...
                    if (time.time() - self.captain_wait_start) >= CAPTAIN_DECLINE_GRACE:
                        print(
                            "[Bot] Captain prompt missing (Kanamia) -> ESC, then I -> LEAVE_PARTY")
                        pyautogui.press('esc')
                        time.sleep(0.2)
                        pyautogui.press('i')
                ...
                # 5) Matching/Confirm loop
                    ...
                    if not self.matching_seen:
                        ...
                        for rp in MATCHING_REGIONS:
                            ...
                            if mlabel:
                                ...
                                break
                        if found:
                            print("[Bot] 'Matching...' seen (Kanamia) -> latched")
                    ...
                    if (now - self.last_match_retry_ts) >= MATCH_RETRY_COOLDOWN:
                        ...
                        if not still_matching:
                            print(
                                "[Bot] Matching missing (Kanamia) -> click MATCH again")
                    ...
                    if cfm:
                        print("[Bot] Match popup (Kanamia) -> initial CONFIRM")
                    ...
                    if (not self.matching_seen) and ((now - self.match_check_start) > 5.0):
                        print(
                            "[Bot] No Matching/Confirm (Kanamia) -> ESC, then I -> LEAVE_PARTY")
                        pyautogui.press('esc')
                        time.sleep(0.2)
                        pyautogui.press('i')
                        self.leave_enter_ts = time.time()
                ...
                # 6) Confirm monitor
                    ...
                    if cfm:
                        ...
                        print(
                            "[Bot] CONFIRM_MONITOR (Kanamia): CONFIRM seen -> click")
                        self._click_at(cfm.center)
                        time.sleep(0.3)
                    ...
                    if vic:
                        print(
                            "[Bot] Victory during confirm-monitor (Kanamia) -> WAIT_VICTORY_LEAVE")
                    ...
                    if (now - self.confirm_monitor_start) >= CONFIRM_MONITOR_DURATION:
                        if (now - self.last_confirm_seen) > CONFIRM_GONE_MARGIN:
                            print(
                                "[Bot] Confirm disappeared (Kanamia) -> start LINKCLICK_SPAM")
                ...
                # 7) Spam
                    ...
                    if self.hud_timer_armed and self.dungeon_timer_start > 0:
                        if (now - self.dungeon_timer_start) >= SPAM_FAILSAFE_SECS:
                            if (now - self.failsafe_last_try_ts) >= FAILSAFE_RETRY_SECS:
                                print(
                                    "[Bot] HUD failsafe reached (Kanamia) -> press P and try Confirm")
                                pyautogui.press('p')
                                time.sleep(0.3)
                                cf = self._scan_full(
                                ...
                                if cf:
                                    print(
                                        "[Bot] Failsafe confirm (Kanamia) -> click, go POST_LEAVE_CHECK")
                    ...
                    if cfm:
                        print(
                            "[Bot] LINKCLICK_SPAM (Kanamia): Confirm reappeared -> click & back to CONFIRM_MONITOR")
                        self._click_at(cfm.center)
                        time.sleep(0.3)
                        self.confirm_monitor_start = time.time()
                        self.last_confirm_seen = time.time()
                    ...
                    vic = self._scan_full(
                        sct, "LBL_VICTORY", THRESH["LBL_VICTORY"])
                    if vic:
                        print("[Bot] Victory detected during spam (Kanamia)")
                        self.state = "WAIT_VICTORY_LEAVE"
                        continue
                ...
                # 8) Victory -> Leave
                    ...
                    if lv:
                        print("[Bot] Leave Dungeon (Kanamia) -> click")
                ...
                # 9) Post-leave sanity
                    ...
                    if cfm:
                        print(
                            "[Bot] Post-Leave (Kanamia): CONFIRM found -> click & back to CONFIRM_MONITOR")
                        self._click_at(cfm.center)
                        time.sleep(0.6)
                        self.confirm_monitor_start = time.time()
                        self.last_confirm_seen = time.time()
                    ...
                    if f:
                        print(
                            "[Bot] Post-Leave (Kanamia): F_KANAMIA detected -> back to WAIT_F")
                        self._full_reset()
                        continue

                    if (now - self.post_leave_start) > POST_LEAVE_WINDOW:
                        print(
                            "[Bot] Post-Leave window elapsed (Kanamia) -> reset to WAIT_F")
                        self._full_reset()
                        continue
                ...
                # 10) Leave-Party flow
                    ...
                    if lp:
                        print("[Bot] Leave-Icon (Kanamia) -> click")
                        ...
                        cf = self._scan_full(
                            sct, "BTN_CONFIRM_PARTY", THRESH["BTN_CONFIRM_PARTY"])
                        if cf:
                            print("[Bot] Confirm party leave (Kanamia) -> click")
                            self._click_at(cf.center)
                            time.sleep(0.8)
                    ...
                    # 2) After 5s without leave icon: ESC -> check F_KANAMIA
                    if (now - self.leave_enter_ts) >= 5.0:
                        print(
                            "[Bot] Leave-Icon not found for 5s (Kanamia) -> ESC and check F_KANAMIA")
                        pyautogui.press('esc')
                        time.sleep(0.2)

                        f = self._scan_full(
                            sct, "F_KANAMIA", THRESH["F_KANAMIA"])
                        if f:
                            print(
                                "[Bot] F_KANAMIA detected after ESC (Kanamia) -> reset to WAIT_F")
                            self._full_reset()
                            continue

                        # one-time asset probe after 5s without F (Kanamia's Mindrealm)
                        if self.leave_probe_start_ts == 0.0:
                            ...
                            print(
                                "[Bot] 5s without Leave-Icon/F (Kanamia) -> run one-time asset probe")
...
def _run():
    ...
    t.start()
    print("Hotkeys: F8 start • F9 pause • F10 exit (Kanamia)")

    def on_press(key):
        try:
            if key == keyboard.Key.f8:
                bot.running = True
                print("[Bot] RUNNING (Kanamia)")
            elif key == keyboard.Key.f9:
                bot.running = False
                print("[Bot] PAUSED (Kanamia)")
            elif key == keyboard.Key.f10:
                bot.exit_flag = True
                print("[Bot] EXIT (Kanamia)")
                return False
        except Exception as e:
            print("[HotkeyError] (Kanamia)", e)
